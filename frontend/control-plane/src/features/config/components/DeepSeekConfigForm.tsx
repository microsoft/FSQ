...
interface DeepSeekConfigFormProps {
  ...
  canTest: boolean;
  testPending: boolean;
  onChange: (draft: DeepSeekConfigPayload) => void;
  onLoadModels: () => void;
  onSave: () => void;
  onCancel: () => void;
...
export function DeepSeekConfigForm(props: DeepSeekConfigFormProps) {
  ...
  const canSave = props.dirty && offered && Boolean(props.draft.apiKey.trim()) && !props.savePending && !props.blocked;
  const locked = props.savePending || props.blocked;
  return <section className="config-editor" aria-labelledby="deepseek-config-title">
    <div className="config-section-heading">
      <div><p className="config-eyebrow">{props.configured ? 'Active provider' : 'New provider'}</p><h1 id="deepseek-config-title">DeepSeek configuration</h1></div>
    ...
    <form className="config-form" aria-busy={props.savePending} onSubmit={event => { event.preventDefault(); if (canSave) { setKeyVisible(false); props.onSave(); } }}>
      <div className="config-field">
        ...
        <span className="config-secret-input">
          <input id="deepseek-api-key" required autoComplete="off" type={keyVisible ? 'text' : 'password'} value={props.draft.apiKey} disabled={locked} onChange={event => { setKeyVisible(false); props.onChange({ modelName: '', apiKey: event.target.value }); }} />
      ...
      <div className="config-field">
        <div className="config-model-heading">
          {hasModels ? <label htmlFor="deepseek-model">Model</label> : <span>Model</span>}
          <small id="deepseek-model-rule">deepseek-flash / Flash V4.1+ / Pro V4+ only; no extra suffixes.</small>
        </div>
        {hasModels ? <select id="deepseek-model" aria-describedby="deepseek-model-rule" required value={props.draft.modelName} disabled={locked} onChange={event => props.onChange({ ...props.draft, modelName: event.target.value })}>
          <option value="">Select a model</option>
          {models.map(model => <option key={model.id} value={model.id}>{model.name}</option>)}
        </select> : props.draft.modelName && <p className="config-model-value">{props.draft.modelName}</p>}
      </div>
