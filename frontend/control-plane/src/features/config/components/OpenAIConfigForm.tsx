...
interface OpenAIConfigFormProps {
  ...
  canTest: boolean;
  testPending: boolean;
  onChange: (draft: OpenAIConfigPayload) => void;
  onLoadModels: () => void;
  onSave: () => void;
  onCancel: () => void;
...
export function OpenAIConfigForm(props: OpenAIConfigFormProps) {
  ...
  return <section className="config-editor" aria-labelledby="openai-config-title">
    <div className="config-section-heading">
      <div><p className="config-eyebrow">{props.configured ? 'Active provider' : 'New provider'}</p><h1 id="openai-config-title">OpenAI configuration</h1></div>
    ...
    <form className="config-form" aria-busy={props.savePending} onSubmit={event => { event.preventDefault(); if (canSave) { setKeyVisible(false); props.onSave(); } }}>
      <div className="config-field">
        ...
        <span className="config-secret-input">
          <input id="openai-api-key" required autoComplete="off" type={keyVisible ? 'text' : 'password'} value={props.draft.apiKey} disabled={locked} onChange={event => { setKeyVisible(false); props.onChange({ modelName: '', apiKey: event.target.value }); }} />
      ...
      {props.models.state === 'ready' && models.length > 0 ? <div className="config-field">
        ...
        <select id="openai-model" required value={props.draft.modelName} disabled={locked} onChange={event => props.onChange({ ...props.draft, modelName: event.target.value })}>
