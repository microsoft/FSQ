...
Web dynamic execution and deterministic Cases use the same self-contained locator contract: a declared page alias and an ordered locator path. Element refs, snapshot-target strings, backend frame IDs, and session-local lookup handles are not supported action inputs or recorded selectors. Explicit container/filter/first/nth rules remain executable rules rather than identities of the elements selected during recording. Locator preparation and replay-parameter validation precede external effects; unavailable replay parameters prevent the action.
...
Historical Run data produced by v0.1.0 and subsequent releases remains readable through built-in CLI and Control Plane workflows. Reports, events, source markers, errors, evidence, and artifact paths retain their original contents; inspection does not rewrite them or re-evaluate the Run. Executable Web Cases must satisfy the current Web parameter contract; ref-bearing targets and incompatible locator shapes are rejected without implicit migration. Current dynamic provenance uses `agent_runtime.runner` and `agent_runtime.verifier`, and generic runtime failures use `agent_runtime_error`. Historical values coexist with current values as persisted data; saved artifact paths are resolved directly, not reconstructed from current labels. Python compatibility aliases and external scripts matching old output strings are not part of this local-data contract.
...
- Platform id: `web`.
- Backend: `playwright`.
- Runtime settings include Workspace browser channel and discovered/configured executable path, preset headless/browser policy, optional base URL, and optional viewport fields.
...
- Element actions accept replayable locator paths with exact-by-default semantic matching, explicit frame/container scope, conjunctive filters, and explicit positional selection. Query ambiguity is an error, not permission to choose the first result. Absence waits and assertions retain their operation-specific cardinality semantics.
- Ordinary Web capabilities include navigation, pointer/drag/scroll, separate fill and sequential input, checked state, native selection, scoped keyboard, condition waits, deterministic assertions, owned-page operations, and trigger-bound popup/dialog handling. File upload, arbitrary JavaScript/code execution, network/storage/DevTools, coordinate fallback, and generic bulk execution are not exposed.
- Web observations are structured page/region/element views containing validated replayable locators, useful control state, and explicit coverage/omission information. `ui_snapshot`, `find_elements`, and `inspect_element` are driver-owned observation/diagnostic capabilities; raw ARIA text and temporary refs are not the action-selection interface.
- The default inline observation body is bounded to 12,000 characters and every model-facing Web tool response to 16,000 characters. Full observations remain Run-contained evidence; model views reuse applicable post-action observations without duplicating full runner results or clipping locator values. The generic context-history limits remain a backstop.
- Harness skill: `web-harness.md`; its guidance and validated examples agree with active tool/schema descriptions on locator scope, selection order, defaults, mutually exclusive arguments, event sequencing, and effect-aware recovery.
- Web integration reuses the existing Runner, Evidence, recording, and publication contracts. The Driver owns browser behavior, Models owns Web tool schemas, and `web-harness.md` owns Web usage guidance. Harness and coding-agent adaptations are limited to Web invocation, observation persistence, and bounded presentation; there is no separate Web execution pipeline, upload service, or new global recording-completeness mechanism.
