...
`agent_engine` owns independently reusable model/provider/agent-execution protocols and neutral input, output, tool, event, filter, and error contracts. Its private implementation uses the pinned OpenAI Python client for Responses requests and owns a local agent execution loop. Concrete backend objects, Responses message shapes, third-party exceptions, and concrete constructor injection do not cross into FSQ business code.

All six inference paths use this boundary: pre-plan, main execution, and verification call `AgentEngine.run`; visual assertions, connection testing, and Case suggestions call tool-free `Model.complete`. The private backend owns the sole tool dispatcher and continuation loop plus direct tool-free Responses requests. Both paths share private protocol conversion without nesting the public single-request operation inside the agent loop or adding semantic retries.

Both engine entry points share private response validation inside `agent_engine`. Known incomplete, failed, or refused responses produce safe neutral `EngineError` failures rather than usable partial results. Business consumers handle these errors through their existing operation-result mappings and do not duplicate provider response-status checks. Agent-specific tool continuation, turn limits, and caller-owned structured-output validation remain in the agent execution loop; direct `Model.complete` requests do not enter that loop.

The supported engine surface consists of bound function tools, caller-owned output contracts, streaming and non-streaming execution, context filtering, and safe tracing export. Runtime and normal test dependencies do not include `openai-agents`; no SDK fallback, complete SDK copy, or runtime reference-checkout dependency is used. Reused upstream engine source carries recorded provenance and applicable notices in source and installed distributions.
...
| agent_engine | fsq_agent/agent_engine/SPEC.md | Owns reusable model/provider/agent protocols, neutral inference contracts, the private Responses backend, local agent execution, and safe tracing export. |
