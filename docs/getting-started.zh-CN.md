...
DeepSeek 固定使用 `https://api.deepseek.com/` 和 Responses API。模型列表接受 `deepseek-flash`、4.1 及以上的精确版本化 Flash ID，以及 4.0 及以上的精确版本化 Pro ID，不设置最高版本；其他模型族和额外后缀会被排除。发现过程不探测能力或图片支持；输入不兼容时正常失败，不自动重试、替换模型或回退 Provider。
