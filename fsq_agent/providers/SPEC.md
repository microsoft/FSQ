...
- `test_model_provider_connection(user_config_root: str | Path | None = None) -> ProviderConnectionTestResult`: Loads the latest saved provider, creates a fresh session, sends one fixed minimal plain-text prompt requesting a short deterministic acknowledgement without an output contract or business JSON schema, returns provider/model/elapsed duration after a valid response, and always closes the session.
...
- Model requests contain neutral text/image input and an optional caller-owned output contract; results contain neutral text/usage/completion facts and an optional typed parsed value. Sessions forward these values without schema interpretation, parser invocation, text reparsing, schema downgrade, or business verdict conversion. Neutral inference errors retain their categories across session forwarding. Existing no-contract text callers remain valid. Model inference does not trigger provider login or refresh; those operations follow the existing explicit preparation policies.
...
Neutral forwarding verification covers synchronous/asynchronous output-contract and parsed-value preservation, no supplier-side parsing, safe engine-error propagation, no-contract text compatibility, and plain-text connection testing with scoped cleanup.
