...
Web capabilities use the shared platform catalog, inherited `wait_ms`, and driver-backed ordinary navigation, pointer/drag/scroll, separate fill/sequential input, checked state, selection, scoped keyboard, condition waits, assertions, owned-page operations, and trigger-bound events. `ui_snapshot`, `find_elements`, and `inspect_element` expose structured observations; screenshot and AI assertion capabilities retain their explicit roles. Canonical snake_case names and authored FSQ aliases come only from capability metadata. Text entry preserves `textType="runtimeSecret"` resolution by `StepRunner`. File upload, arbitrary JavaScript/Playwright-code execution, network/storage/DevTools, PDF/download management, coordinate fallback, and generic bulk execution are not exposed.
...
Web element operations accept self-contained `WebLocator` paths with a declared page alias, semantic/selector steps, conjunctive filters, explicit position, and frame entry. Refs, free-text targets, runtime IDs, and locator bags are rejected. Exact matching is the default; display truncation never alters a locator. Explicit first/nth rules are preserved and re-evaluated, not replaced with fixed identities. The Driver prepares replayable locators before its action primitive, uses bounded readiness/actionability checks, rejects ambiguity without implicit first-match selection, and applies condition-specific cardinality for absence waits/assertions.

Web observations supply region/control structure, directly usable checked locators, relevant state/options, and explicit coverage/omission/continuation information. Raw ARIA text is not the model-facing targeting contract. Harness persists full observations through Core Evidence and exposes bounded views with artifact references; the coding-agent adapter owns the 16,000-character model response projection. No observation/query or screenshot helper becomes a generated action command.

Web failures retain shared failure categories plus Driver-supplied precise diagnostics in existing metadata. Trigger listeners precede popup/dialog operations. Backend semantics stay inside Drivers/Harnesses rather than Web-name switches, new phases, or recording-state policies in Runner.

Web owns public `WebDriverInterface`, private `WebHarness`, private `PlaywrightWebDriver`, catalog-backed declarations, and default capability definitions selected through public factories. The interface extends the shared observation contract with typed ordinary actions and structured observations. Automatic runner capture writes `screenshot` and normalized `ui_snapshot` artifacts with preserved coverage. AI assertions use injected shared backend support. Playwright import and configured Chromium distribution launch are explicit `start_browser` concerns, not construction or registry-bootstrap effects. The driver passes validated `channel` and `executable_path` to the Chromium launcher; Firefox and WebKit are not selected through this channel contract. Final disposal reuses owned cleanup without creating an authored `closeBrowser` event.
