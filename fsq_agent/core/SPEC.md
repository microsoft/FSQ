...
Web element interactions accept only a model-validated semantic locator. A locator requires an ARIA role and may include an accessible name, one non-recursive role/name `within` scope, and a non-negative zero-based `index` applied only to the final target match set. The parent scope must resolve uniquely and cannot be disambiguated by the target index. Complete names use whole-string case-sensitive matching; names ending in literal `...` use an escaped, anchored, case-sensitive prefix match. Resolution reads current page semantics directly and never depends on snapshot refs, handles, caches, or a preceding `ui_snapshot` call. Positive resolution distinguishes no match, ambiguous match, an out-of-range index, hidden or detached targets, and backend interaction failures through bounded structured diagnostics.

Web `select_option` accepts one non-empty ordered list of unique visible labels and rejects multiple labels for a single-select target. Web `wait_for` accepts exactly one semantic locator or URL condition; element state defaults to visible, timeout only bounds a supplied condition, and hidden or detached waits reject indexed locators. Web `assert_not_visible` rejects indexed locators and passes when no visible target exists, including an absent unique parent scope, while multiple visible parent scopes remain ambiguous. Web `assert_text` requires exactly one literal `contains` or `equals` predicate and reads either one resolved semantic target or, when no locator is supplied, the current page body without additional text normalization.

Web owns public `WebDriverInterface`, private `WebHarness`, private `PlaywrightWebDriver`, Web catalog-backed platform declarations, and Web default capability definitions selected through public factories. `WebDriverInterface` extends the shared driver observation contract and exposes `ui_snapshot(WebUiSnapshotParams)`; automatic runner capture writes `screenshot` and normalized `ui_snapshot` artifacts. Web `ui_snapshot` uses Playwright default ARIA output when a page is active, emits no snapshot refs, preserves valid snapshot structure while compacting semantic values longer than 100 Unicode characters, and otherwise returns the existing ref-free text fallback. `PlaywrightWebDriver.ui_snapshot` is the decorated explicit observation capability for authored dynamic and strict commands. `PlaywrightWebDriver.assert_with_ai` is a decorated backend tool that calls shared AI assertion support. Playwright import and configured Chromium distribution channel plus executable launch are explicit `start_browser` runtime/backend concerns, not driver-construction or registry-bootstrap concerns. The driver passes both validated `channel` and `executable_path` to Playwright's Chromium launcher. Firefox and WebKit browser types are not selected through this channel contract. `PlaywrightWebDriver.close()` remains a final resource cleanup hook and must use the same close implementation without turning cleanup into a task-visible `closeBrowser` event.
