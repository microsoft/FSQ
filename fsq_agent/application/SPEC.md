...
- Provider operations support user-level OpenAI, DeepSeek, and Google Gemini model discovery/configuration, Azure OpenAI configuration, GitHub Copilot device authorization/model activation, and active-Provider readiness status. Supplier model discovery does not expose a Provider profile inventory.
...
`list_deepseek_models(*, api_key: str)` and `configure_deepseek(*, model: str, api_key: str, user_config_root: str | Path | None = None)` are public Provider-resource and package exports with the same candidate-only boundary. Listing delegates to Providers and returns immutable safe `DeepSeekModel` facts in provider-defined order without persistence, capability probing, or inference. Configuration re-runs complete discovery with the submitted key and requires exact selected-id membership before calling `save_deepseek_provider`; an empty eligible set or absent id is `model_not_offered`. Results reuse `ProviderConfigurationResult(provider="deepseek", model=...)` without credentials.

`list_google_gemini_models(*, api_key: str)` and `configure_google_gemini(*, model: str, api_key: str, user_config_root: str | Path | None = None)` are public Provider-resource and package exports with the same candidate-only boundary. Listing delegates complete bounded pagination and filtering to Providers and returns its immutable `GoogleGeminiModel` facts. Configuration re-runs discovery with the submitted key and requires exact normalized model-id membership before calling `save_google_gemini_provider`; no partial collection authorizes activation. A complete empty collection or missing selected id is `model_not_offered`, while incomplete discovery retains its failure rather than becoming a membership rejection. Results reuse `ProviderConfigurationResult(provider="google_gemini", model=...)` without credentials. Provider status and existing Case operations consume Gemini through the same neutral supplier/session boundary without Google protocol knowledge.
...
DeepSeek list/configure operations use the OpenAI/Gemini error categories and allowlisted reasons with `details.provider="deepseek"` and operation-specific safe messages/actions. Invalid model metadata cannot become partial success. Safe details contain only provider and reason; keys, upstream payloads, and unrestricted exception context are excluded. Lost save responses require current-configuration reconciliation rather than automatic resubmission or assumed rollback.
...
- The Provider boundary has one active Provider and no profile inventory, retained profiles, fallback chain, or transport-specific UI models. OpenAI, DeepSeek, Gemini, and GitHub model discovery enumerate eligible model facts only and do not activate a Provider.
