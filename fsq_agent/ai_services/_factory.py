...
def build_ai_assertion_evaluator(settings: Settings) -> AIAssertionEvaluator:
    return AIAssertionEvaluator(build_model_provider_session(settings), reasoning_effort=settings.agent_runtime.reasoning_effort)


def build_case_suggestion_analyzer(settings: Settings) -> CaseSuggestionAnalyzer:
    return CaseSuggestionAnalyzer(build_model_provider_session(settings), reasoning_effort=settings.agent_runtime.reasoning_effort)
...
def check_case_suggestion_readiness(settings: Settings) -> tuple[bool, str, str]:
    ...
    try:
        ...
        CaseSuggestionAnalyzer(session, reasoning_effort=settings.agent_runtime.reasoning_effort)
