...
```text
fsq
├── init [--platform PLATFORM] [--agent codex]
├── doctor
├── case
│   ├── create
│   ├── test
...
Codex agent initialization is independent of Workspace initialization and may be combined with it:
...
At least one of `--platform` or `--agent` is required. `--agent` accepts only `codex`. Without `--platform`, Workspace and platform target options are rejected; the Codex project is the exact current directory and the intended Workspace defaults to `<current-directory>/workspace` for result reporting. With both options, CLI completes Workspace initialization first and passes the returned actual Workspace root to the independent Application Coding Agent installation operation; the Codex project remains the exact current directory. Both Codex TOML resources and the `AGENTS.md` workflow content are installed without content interpretation or project/Workspace path rendering. A Workspace result remains valid if subsequent Coding Agent installation fails, and the error states that initialization may already have completed rather than claiming rollback. Any existing `AGENTS.md` directory entry is always skipped while the two Codex TOML destinations retain normal create/update behavior. `--update-agents-md` is not a public option. `--no-overwrite` skips every existing TOML destination and is rejected without `--agent`.

When `--platform` is supplied, `init` accepts the same complete platform target shapes as Control Plane: Android requires `--app-id`; Web requires `--browser-channel` while `--browser-executable-path` is optional; Windows requires `--app-path` and accepts `--window-title-re` and `--launch-args`; macOS requires at least one of `--bundle-id` and `--app-path`. Target options for another platform are rejected. When the Web path is omitted, Application discovers an executable compatible with the exact selected channel on the current host. One unambiguous compatible candidate is selected; no candidate or multiple distinct candidates fail before workspace mutation with safe guidance to install the channel or pass an explicit path. An explicit path is normalized and validated against the channel through the same shared operation.

Platform initialization always checks selected-platform runtime readiness before workspace mutation. Readiness checks are read-only. A missing Python platform dependency reports that the installed `fsq-agent` distribution is incomplete and must be reinstalled or repaired; a missing host service, browser/application target, device prerequisite, or other system dependency reports a safe external provisioning action. Agent-only initialization performs no platform readiness check. `init` does not expose `--install-driver` or any runtime-install option and never invokes Python or system package managers, installs or modifies the application under test, creates emulators/VMs, connects to a device, starts Appium, or authenticates.
...
`fsq init --platform` is the only CLI form that may establish this precondition. It validates all platform input, resolves the complete target, and completes Driver readiness before workspace mutation; it then creates an unregistered Workspace from the current selected directory or initializes and updates exactly one platform at an existing registered root through the shared Application and Config operations. An empty selected directory is adopted as the new root; a non-empty selected directory is preserved and receives an absent `<current-directory>/<name>` child. Success is reported only after workspace files and registry truth are committed. It creates `.fsq/config/config.<platform>.yaml`, `.fsq/runs/<platform>/`, `cases/<platform>/`, `knowledge/<platform>/project.md`, and the user registry entry inside the selected final root. Repetition for an existing registered name is idempotent and uses its stored root independently of the current directory. A partially initialized, unavailable, mismatched, invalid, or Driver-unready Workspace fails with safe repair guidance rather than being treated as ready. `fsq init --agent codex` alone does not establish or imply Workspace registration.
...
OpenAI configuration uses only the official endpoint and rejects `--base-url`. Human interactive mode securely prompts for an omitted API key, loads the eligible models through Application, and requires selection from that set when `--model` is omitted. A valid empty set produces actionable unavailable guidance without an empty selection prompt or a default. An explicit model must also be in the eligible set. Application revalidates membership before atomically saving the complete candidate; configuration sends metadata requests but no model inference. JSON, JSONL, and `--non-interactive` require both `--model` and `--api-key` and never prompt. Missing or provider-inapplicable options are usage errors; rejected candidates/models and confirmed storage failures use configuration errors; supplier unavailability uses unavailable errors; unexpected or unconfirmed persistence failures use internal errors. Output exposes no key or raw supplier response. A lost/interrupted outcome is not a cancellation guarantee and is never retried automatically.

DeepSeek configuration uses only the official endpoint and rejects `--base-url`. It follows the same secure key prompt, explicit offered-model selection, valid-empty handling, current-set revalidation, no-inference discovery, machine/non-interactive requirements, lost-outcome behavior, and exit categories as OpenAI. Results identify `provider="deepseek"`; output exposes no key, upstream payload, or capability claim beyond local model-selection eligibility.

Google Gemini configuration uses the same interactive/non-interactive and exit-category contracts through Application's Gemini list/configure operations. Human mode masks an omitted key's prompt and requires an explicit selection from the complete eligible discovery result when the model is omitted, even if there is one choice. A complete empty collection is unavailable; pagination failure is not an empty successful result. JSON, JSONL, and `--non-interactive` require both model and key without prompting. Explicit models are revalidated against the current complete set before activation. `--base-url` is rejected because Gemini uses only the official Developer API endpoint. Results identify `provider="google_gemini"` without key, page-token, SDK, or raw supplier data. Configuration requires no Workspace and sends no inference; status and Case workflows use the same saved Provider as Control Plane.

Kimi configuration uses one `kimi` Provider with required region `cn` or `global`. Human mode prompts for region with no default, securely prompts for an omitted key, discovers from the matching fixed official regional endpoint, and requires explicit model selection even for a one-item result. JSON, JSONL, and `--non-interactive` require region, model, and key without prompting. Explicit models are revalidated by fresh discovery using the same region/key pair before activation. `--base-url` is rejected; keys are never retried against the other region. Results and status identify `provider="kimi"`, region, and model without key, endpoint, SDK, or raw supplier data. Configuration requires no Workspace and sends no inference.
...
`providers status` loads the real active user Provider and model, checks local authentication and non-interactive readiness through Application, and never starts device login, discovers models, or sends model inference. It returns one result with `status` (`ready` or `unavailable`), `configured`, `provider`, optional Kimi `region`, `model`, `authenticated`, a safe `message`, and a safe repair `action` when needed. The configured Provider value is `openai`, `azure_openai`, `deepseek`, `google_gemini`, `github_copilot`, or `kimi`. Human, JSON, and JSONL expose equivalent facts; JSONL emits one terminal record and no synthetic event. Ready exits `0`; a completed unconfigured, unauthenticated, or otherwise unavailable result exits `4`. Invalid user configuration is a configuration error, and unrecoverable orchestration failure is an internal error. Output never includes tokens, API keys, authorization data, raw backend responses, exception messages, or tracebacks.
