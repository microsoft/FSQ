...
- `doctor`: Runs readiness diagnosis, closed safe repair, mode-specific aggregation, and text/JSON rendering.
...
- `fsq-agent doctor [--platform android|web|windows|macos] [--mode dynamic|strict|all] [--format text|json] [--color auto|always|never] [--non-interactive] [--repair]`: Diagnose the current directory's Python/install, preset/`.env`, workspace, provider, and selected platform connectivity. An explicit platform wins; otherwise effective platform environment-variable presence selects one candidate. When both stdin and stdout are TTYs, doctor prompts for unresolved platform and omitted mode (`all` default) and offers eligible repairs. Text output streams `RUNNING` and completed status events immediately; TTY output updates the running line in place and plain output appends lines. `--color` defaults to `auto`, supports forced/disabled ANSI styling, and never removes status words. Redirected/CI execution and `--non-interactive` never prompt. `--repair` applies eligible no-input repairs without confirmation; non-secret `.env` values are collected only through interactive prompts. JSON implies non-interactive diagnosis, is incompatible with repair, writes one clean schema-versioned document to stdout with no progress/ANSI data, and never mutates state. Doctor returns `0` when requested readiness targets are ready, `1` when blocked, `2` for invalid/unresolved usage, and preserves user interruption as `130`.
...
Doctor rejects JSON with repair and unresolved non-interactive platform detection with exit code `2` while still emitting valid selected-format output. JSON also rejects forced progress/color output and contains no ANSI sequences. Interactive prompts require both stdin and stdout TTYs. Doctor ordinary check/repair failures are report data rather than uncaught CLI errors, and JSON output is not mixed with logging.
