...
Provide the public command line surface for fsq-agent: initialize local setup, run TTY-aware doctor diagnosis/safe repair, execute dynamic or strict tasks, print reports, and start the local browser playground. CLI remains a thin transport over owning module APIs.
...
- `doctor`: Runs complete diagnosis, closed safe repair, one overall result, and text/JSON rendering.
...
- `fsq-agent init --platform android|web|windows|macos [--provider github_copilot|azure_openai]`: Initialize or verify the current directory `.fsq-agent-workspace` for one committed platform preset, validate static configuration, and print readiness for the default LLM run path, strict-core run path, and provider-backed AI assertion readiness. Static configuration or workspace failures exit nonzero. Mode-specific dependency gaps are reported as readiness failures so strict-only users are not forced to configure LLM credentials unless they execute cases containing `assertWithAI`. When `--provider` is omitted, `init` must not mutate provider settings or start provider authentication. When `--provider` is supplied, `init` also updates local provider settings in the current directory `.env`, prepares provider-local readiness, and may prompt for Azure values or trigger Copilot device-code authentication as described below.
- `fsq-agent doctor [--platform android|web|windows|macos] [--format text|json] [--color auto|always|never] [--non-interactive] [--repair]`: Diagnose the current directory's Python/install, preset/`.env`, workspace, provider, and selected platform connectivity. An explicit platform wins; otherwise effective platform environment-variable presence selects one candidate. Every valid diagnosis runs the complete provider and selected-platform check set and computes one overall result from final check statuses. Human-readable output gives each concrete check one bracketed title derived from its stable id, places its Check, Action, Repair, Verify, Result, Fix, Run, and Backup information beneath it, and does not render duplicate phase headings. When both stdin and stdout are TTYs, Doctor handles each actionable issue before dependent work continues: it displays `ACTION REQUIRED` instead of a premature final severity, prompts once, removes transient action content after the decision, immediately applies and verifies an accepted allowlisted repair, then leaves only the final section state. TTY output updates transient lines in place; redirected/plain output is append-only but retains the same per-check hierarchy. Redirected/CI execution and `--non-interactive` never prompt or display `ACTION REQUIRED`; they display final states directly. `--repair` applies and immediately verifies eligible no-input repairs without confirmation and skips input-required repairs. Final text contains `Summary: PASS|FAIL|ERROR|CANCELLED` followed by aggregate check counts and has no readiness or impact section. `--color` defaults to `auto`, supports forced/disabled ANSI styling, and never removes status words. JSON implies non-interactive diagnosis, is incompatible with repair, writes one clean schema-versioned document to stdout with no display titles, repair side effects, transient action-required data, progress/ANSI, diagnostic-mode, target-readiness, or affected-target data, and never mutates state. Doctor returns `0` when no final check failed, `1` when any final check failed, `2` for invalid/unresolved usage, and preserves user interruption as `130`. The public command does not expose `--mode`; invocations using it fail as an unknown option.
...
Public CLI commands do not expose `--config` or `--workspace`; invocations using those options fail as unknown options.

`init --provider` writes only managed provider keys in the current directory `.env`: `FSQ_LLM_PROVIDER`, `AZURE_OPENAI_BASE_URL`, `AZURE_OPENAI_MODEL`, and `AZURE_OPENAI_API_KEY`. It must preserve unrelated `.env` lines and comments, replace existing assignments for managed keys, and append missing managed keys. Copilot setup writes only `FSQ_LLM_PROVIDER=github_copilot` and must not delete existing Azure variables. Azure setup writes `FSQ_LLM_PROVIDER=azure_openai` plus the fixed Azure variables. Existing process environment values take precedence over `.env`; when an effective process value differs from the `.env` value being written or checked, CLI output must warn that the process environment wins and validate against the effective value.

`init --provider github_copilot` validates provider settings, ensures interactive Copilot auth readiness through public `providers` APIs, and reports local setup readiness without sending a Responses API model request. If no valid cached GitHub OAuth token is available, it runs the GitHub device-code flow and caches the token under `.fsq-agent-workspace/auth/github-copilot-token.json`.

`init --provider azure_openai` prompts for `AZURE_OPENAI_BASE_URL`, `AZURE_OPENAI_MODEL`, and `AZURE_OPENAI_API_KEY`. Endpoint and model prompts may show existing non-secret defaults; API key input must be hidden and may keep an existing non-placeholder effective key without echoing it. Azure readiness validates endpoint normalization to the `/openai/v1/` form, model presence, API key presence, and placeholder-key rejection. It must state that this is local configuration readiness, not proof that the Azure deployment authorizes model calls.

`--goal`, `--case-yaml`, and `--case-dir` are mutually exclusive. `--strict --goal` is invalid because strict-core execution requires authored YAML steps. `--strict --record` and `--strict --record-on-failure` are invalid because recording is a dynamic-run post-processing workflow. `--record-on-failure` without `--record` is invalid. Relative case paths resolve against `cases.dir` first, then the current working directory. Strict hook `runCase` paths from either config-level or case-level hooks use the same relative path policy.
...
- Dynamic recording records by-design observation skips such as Android `uiTree`/ `ui_snapshot`, Web `pageSnapshot`/`page_snapshot`, desktop `uiSnapshot`/`ui_snapshot`, and screenshot/snapshot tools in `recording.json` audit data without adding generated case warnings.canonical
...
- Web strict runs capture automatic `screenshot`/`ui_snapshot` evidence through the centralized driver step-kind policy when the active Web driver has a started page. Explicit `pageSnapshot` commands remain valid authored observations.
...
- `_llm_setup.py`: Internal provider setup orchestration for `init --provider`, including current-directory setup path selection, provider settings loading, provider readiness calls, prompt coordination, and non-secret status summaries.
- `_env_file.py`: Compatibility wrapper over config-owned `.env` document APIs used by provider setup; file parsing, backups, and atomic update mechanics belong to `config`.
- `_doctor.py`: Thin command adapter for TTY/color detection, option compatibility, DoctorRequest construction, progress renderer/sink composition, invoking `DoctorService`, emitting JSON or final text summary, and mapping the report exit code.
...
- Package-private `fsq_agent._strict_case_recording`: Shared post-run recorder used by CLI and Playground to convert dynamic run capability events into run-local `recorded.codex.yaml` and `recording.json` artifacts.
...
- Internal modules: all CLI `_*.py` files are private command/helper implementation modules. Shared dynamic-run recording composition lives in package-private `fsq_agent._strict_case_recording` and is consumed only by CLI and Playground entry layers.
- Domain boundaries: CLI owns argument validation, TTY detection, current-directory selection, provider setup prompts, entry-mode orchestration, registry bootstrap, strict lifecycle orchestration, dynamic recording, output emission, and exit behavior. Config owns `.env`/workspace mutation mechanics; doctor owns diagnostic/repair policy and rendering. CLI does not own checks, safe-repair eligibility, capability implementation, provider internals, config parsing, or report generation.
...
Input validation failures, including unsupported `setup` commands, unsupported public `--workspace` options, missing input source, multiple input sources, `--strict --goal`, invalid record flag combinations, unreadable dynamic case files, invalid strict YAML, malformed config-level or case-level lifecycle hook metadata, empty case directories, missing hook `runCase` files, recursive hook chains, missing strict Android app id from `FSQ_ANDROID_APP_ID` or FSQ case metadata when the active platform is Android, invalid Web navigation/base URL policy when the active platform is Web, invalid or missing Windows app path/pywinauto adapter settings when the active platform is Windows, invalid or missing macOS Appium/target settings when the active platform is macOS, missing strict replay secret allowlist/presence, missing provider readiness for authored strict `assertWithAI`, unresolved reports, or missing generated Playground frontend assets must fail before external UI actions begin for the affected command, case, or hook. Dynamic `--case-yaml` input must not fail merely because the file is invalid YAML, because that path does not parse YAML.

Doctor rejects JSON with repair and unresolved non-interactive platform detection with exit code `2` while still emitting valid selected-format output. JSON also rejects forced progress/color output and contains no ANSI sequences or transient action-required events. Interactive prompts require both stdin and stdout TTYs. Doctor ordinary check/repair/verification failures are report data rather than uncaught CLI errors, and JSON output is not mixed with logging.
...
Provider setup failures during `init --provider`, including invalid `--provider`, unsupported `FSQ_LLM_PROVIDER`, `.env` read/write errors, non-interactive terminals with missing required prompt values, Azure endpoint/model/API-key validation failures, placeholder Azure API keys, process-environment conflicts that make the effective provider invalid, or Copilot auth/token exchange failures must fail with concise diagnostics and no secret values. `init` without `--provider` must not write provider `.env` values or start interactive provider authentication.

Recording failures happen after a dynamic run and must not change that dynamic run's status. The CLI should log and summarize recording errors, including no replayable commands, runtime-secret text references with missing names, unsupported replay commands, generated YAML validation failures, and existing `recorded.codex.yaml` conflicts. Directory runs continue after per-case recording failures.

## Verification Scope

- Verification covers CLI argument validation, doctor TTY/option/color/progress/exit/stdout behavior, JSON cleanliness, provider setup, dynamic goal/raw-case task construction, strict-case execution entry behavior, strict lifecycle ordering/failure semantics, dynamic recording handoff, report lookup, and live event formatting.
...
- The public command surface is limited to `init`, `doctor`, `run`, `report`, and `playground`. Unsupported command names are not retained as compatibility aliases.
- Provider setup is an optional `init --provider` behavior so one command initializes the current directory workspace and, when requested, local model-provider readiness. `init --platform <platform>` without `--provider` remains a platform-preset workspace/readiness check and must not mutate provider settings or start provider authentication. Public commands intentionally do not accept `--workspace` or `--env-file`; they use the current working directory `.env` and `.fsq-agent-workspace` so the setup target is explicit.
- CLI owns prompt coordination for provider setup but uses config-owned `.env` parsing and atomic update mechanics. Provider selection normalization and validation belong to `config`; Copilot device-code authentication, token cache interpretation, Copilot token exchange, and Azure/Copilot client construction belong to `providers`.
- Doctor interaction is an operator-local CLI workflow, not human approval inside dynamic/strict execution. It never prompts for secrets, and its `--repair` path cannot execute arbitrary remediation commands.
