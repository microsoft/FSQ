...
- Shared models contain no SDK runtime objects. They hold serializable configuration, tool/capability metadata, assertion request/results, and platform schemas; the private engine owns SDK construction. `OpenAIAgentsSettings.provider` and related private runtime values are resolved by Config from the active user-provider snapshot, with `None` representing unconfigured state. Provider credentials and model selection have no workspace/environment fallback.
