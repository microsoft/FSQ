...
- `AgentRuntimeSettings`: Pydantic model for tracing, turn limits, neutral reasoning effort, prompt templates, tool-output policy, and effective provider runtime values. Provider accepts `openai`, `azure_openai`, `google_gemini`, `github_copilot`, or `kimi`, defaults to `None`, and model defaults to an empty string until Config applies a complete user-provider snapshot. Every Provider uses the explicitly saved nonempty model. API keys, Kimi region, and token state remain private runtime values resolved from the user configuration/auth store under `~/.fsq`, not provider environment variables or workspace YAML. Config resolves the active Provider record while Providers maps Kimi region `cn` or `global` to its fixed official endpoint. Private `agent_engine` backends own the inference protocols; Models owns no authentication, discovery, SDK, endpoint, or persistence behavior.
