...
Own shared FSQ domain data structures, unified capability metadata contracts, invocation/result contracts, registry snapshot models, strict replay reference models, configuration value models, AI assertion request/result metadata, skill metadata, report metadata, and domain exception classes. Independently reusable inference contracts and errors are owned by `agent_engine`, immutable suggestion-service results by `ai_services`, and resource-specific application/execution contracts by their documented owners; those types do not duplicate the shared domain contracts here.
...
- `GoalKeyAction`: Pydantic model for one ordered key action generated from a goal/reference task and available pre-plan knowledge.
...
- `AgentToolDefinition`: Pydantic model describing one dynamic-only AgentTool helper exposed through the neutral agent runtime. AgentTool definitions are not capability definitions and do not contain replay policy.
- `AgentToolCall`: Pydantic model describing one SDK-neutral AgentTool invocation request against a canonical helper name.
- `AgentToolResult`: Pydantic model describing a normalized AgentTool invocation response before it is serialized for the agent runtime and run events.
...
- `MacOSPressKeyParams`: Pydantic model for `press_key` parameters with one required character or named key and an optional modifier list. Its field descriptions list the named keys and modifiers accepted by the Appium Mac2 backend so LLM callers can author valid shortcuts.
...
- `MacOSUiSnapshotParams`: Pydantic model for the read-only `ui_snapshot` macOS observation capability. It accepts optional maximum depth and simplification flags, rejects platform-specific element-query fields, and returns a bounded compact semantic Appium Mac2 page-source/control-tree snapshot without changing the established tree response fields.
...
- `AIAssertionResult`: Pydantic model describing one provider-backed platform visual assertion outcome. Its existing fields include status/pass boolean, explanation, confidence when available, provider/model metadata, token/latency diagnostics when safe, error, and evidence artifact references. A validated positive verdict has `status="passed", passed=True`; a validated negative verdict has `status="failed", passed=False`. Generation/validation failure has `status="error", passed=False`, where status is authoritative and the boolean is not a negative evaluation. Error text is generic and safe; `metadata["engine_error"]` may contain a neutral engine category and available safe reason alongside provider metadata. No new top-level field, raw model content, validator details, SDK objects, secret values, or hidden model reasoning is part of this result contract.

Shared settings exports:

- `AgentRuntimeSettings`: Pydantic model for tracing, turn limits, neutral reasoning effort, prompt templates, tool-output policy, and effective provider runtime values. Provider accepts `openai`, `azure_openai`, `deepseek`, `google_gemini`, `github_copilot`, or `kimi`, defaults to `None`, and model defaults to an empty string until Config applies a complete user-provider snapshot. Every Provider uses the explicitly saved nonempty model. API keys, Kimi region, and token state remain private runtime values resolved from the user configuration/auth store under `~/.fsq`, not provider environment variables or workspace YAML. Config resolves OpenAI's, DeepSeek's, and Gemini's fixed official endpoints and Azure's saved endpoint, while Providers maps Kimi region `cn` or `global` to its fixed official endpoint. Private `agent_engine` backends own the inference protocols; Models owns no authentication, discovery, SDK, endpoint, or persistence behavior.
...
- `AgentSettings`: Pydantic model for agent-level execution defaults such as step timeout. Model selection belongs to `AgentRuntimeSettings`, and inactive loop/retry knobs are not part of the public YAML surface.
...
- macOS structured observation has one tree-only `ui_snapshot` contract. Shared AgentTool artifact search and slice helpers remain the dynamic-agent path for inspecting externalized large snapshot output; they are not replay commands or macOS driver modes.
...
- Dependency direction: `models` imports no project modules. FSQ business consumers import its public domain symbols through `fsq_agent.models`; the independently reusable `agent_engine` does not import these business contracts.
...
Domain exceptions defined here inherit from `FsqAgentError` and carry concise human-readable messages with optional structured context. Other modules reuse these domain errors. Independently owned engine errors and documented application/execution boundary errors remain at their owning boundaries and are translated to domain or presentation errors where required.
...
- The agent final output contract is model-owned. The runtime always supplies the current `AgentFinalOutput` schema and parser to the neutral engine output contract. The schema version is emitted in the final output for traceability, but schema selection is not a user-facing configuration.
...
- Tool-call reporting uses `ToolCallRecord` for real AgentTool, CommonTool, and PlatformTool invocations. Runtime/provenance records such as progress events, pre-plan reconstruction, agent-runtime summaries, and provider session setup are not represented as real tool calls.
...
- Tool-output settings are internal runtime defaults. At most the three most recent outputs remain inline within both the per-output and cumulative character budgets; older or oversized outputs are written to run-local artifacts and represented by concise references without repeated previews. The context policy does not interpret tool sensitivity markers; existing runtime-secret boundaries redact secret values before they reach it. No user-turn trimming settings are part of the runtime models.
- Runtime prompt text is template-owned through `AgentPromptConfig`. The agent runtime assembles prompt models for project knowledge, successfully loaded skills, task input, and variables, then renders Jinja template files. Static behavioral text, headings, loops, and formatting live in templates. The `AgentFinalOutput` schema and parser supply a neutral engine output contract, not duplicated prompt text or a business-layer SDK object. There is no custom-instruction prompt channel; project-specific guidance belongs in `knowledge/project.md`, and reusable execution guidance belongs in configured skills.
...
- AgentTool request/result models are serializable and SDK-neutral. Tools expose neutral `agent_engine` bindings; private engine backends own protocol-specific function-tool conversion. Shared business models do not import SDK or engine runtime objects.
- AI assertion request/result models are serializable execution evidence. They describe explicit authored platform assertions, provider-backed verdicts, and generation errors; they do not represent locator fallback, testcase mutation, recovery, or hidden model reasoning. Model-facing assertion schemas and parsing belong to `ai_services`, not to these public execution-owned records; safe engine metadata is plain serializable data and adds no engine dependency here.
...
- Page knowledge is represented as a compact graph-like Markdown/JSON format owned by shared models so external generators can produce compatible optional pre-plan files. `index.md` is a concise JSON index for page lookup; each `pages/*.md` file contains one JSON page node. Page identifiers are semantic descriptions without locators. Element locators are explicitly reference locators, not authoritative runtime truth.
- Internal dynamic goal planning is represented separately from execution results. It produces ordered key actions from a goal/reference task, loaded skills, optional project knowledge, optional page index knowledge, and on-demand page knowledge, but it does not execute UI actions or verify runtime state.
- Shared models contain no SDK runtime objects. They hold serializable configuration, tool/capability metadata, assertion request/results, and platform schemas; the private engine owns concrete inference-client and protocol construction. `AgentRuntimeSettings.provider`, optional Kimi region, and related private runtime values are resolved by Config from the active user-provider snapshot, with `None` representing unconfigured state. Provider credentials, region, and model selection have no workspace/environment fallback. Runtime settings and prompt models use only their canonical public names, without SDK-prefixed compatibility exports.
...
- Parsed FSQ `*.fsq.yaml` models are used for existing-Case testing, including lifecycle hook metadata. Adapters pass Case requests to Application and do not reinterpret Case YAML as raw dynamic planning input. `*.codex.yaml` is a temporary compatibility suffix only.
- Platform action parameter schemas come from decorated capability metadata and shared parameter models registered in the capability registry. Exposed CommonTool and PlatformTool parameter models provide concise LLM-facing field descriptions and model-level descriptions for cross-field rules, while Pydantic validators remain the final runtime enforcement layer for invalid argument combinations. There is no MCP schema validation fallback or MCP-derived tool schema source.
