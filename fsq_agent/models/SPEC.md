...
- `AgentRuntimeSettings`: Pydantic model for tracing, turn limits, prompt templates, tool-output policy, and effective provider runtime values. Provider accepts `openai`, `azure_openai`, or `github_copilot`, defaults to `None`, and model defaults to an empty string until Config applies a complete user-provider snapshot. Every Provider uses the explicitly saved nonempty model. API keys and token state remain private runtime values resolved from the user configuration/auth store under `~/.fsq`, not provider environment variables or workspace YAML. Config resolves OpenAI's fixed official endpoint and Azure's saved endpoint. The private `agent_engine` backend owns the inference protocol; Models owns no authentication, discovery, or persistence behavior.
...
- AgentTool request/result models are serializable and SDK-neutral. Tools expose neutral `agent_engine` bindings; the private engine owns Responses function-tool conversion. Shared business models do not import SDK or engine runtime objects.
