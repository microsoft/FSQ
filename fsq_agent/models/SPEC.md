...
- `AgentRuntimeSettings`: Pydantic model for tracing, turn limits, neutral reasoning effort, prompt templates, tool-output policy, and effective provider runtime values. Provider accepts `openai`, `azure_openai`, `google_gemini`, or `github_copilot`, defaults to `None`, and model defaults to an empty string until Config applies a complete user-provider snapshot. Every Provider uses the explicitly saved nonempty model. API keys and token state remain private runtime values resolved from the user configuration/auth store under `~/.fsq`, not provider environment variables or workspace YAML. Config resolves OpenAI's and Gemini's fixed official endpoints and Azure's saved endpoint. Private `agent_engine` backends own the inference protocols; Models owns no authentication, discovery, SDK, or persistence behavior.
