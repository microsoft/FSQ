...
- `AgentRuntimeSettings`: Pydantic model for tracing, turn limits, prompt templates, context trimming, AgentTool output policy, and effective provider runtime values. Provider defaults to `None` and model to an empty string until Config applies a complete user-provider snapshot. Both Azure and Copilot use the explicitly saved nonempty model; endpoint, API key, OAuth token, and provider-token state come from the user configuration/auth store under `~/.fsq`, not provider environment variables or workspace YAML. The private `agent_engine` backend owns the inference protocol.
...
- `ContextTrimmingSettings`: Pydantic model controlling model-input trimming for older large tool outputs, including recent turn retention, maximum inline tool output size, preview size, and optional trimmable tool names. These values are internal runtime defaults and are not part of the default YAML surface.
- `LocalToolOutputSettings`: Pydantic model controlling how local AgentTools write full outputs to per-run artifacts and decide whether model-facing responses contain full output or artifact references. These values are internal runtime defaults and one-option policy fields should not be exposed as YAML knobs.
