...
def test_web_parameter_models_produce_canonical_dumps_and_reject_extra_fields() -> None:
    ...
    click = WebClickOnParams.model_validate({"locator": {"role": "button", "name": "Sign in"}})
    typed = WebTypeTextParams.model_validate({"locator": {"role": "textbox", "name": "Search"}, "text": "bing.com"})
    wait = WebWaitForParams.model_validate({"locator": {"role": "main", "name": "Results"}, "state": "visible", "timeout_ms": 5000})

    assert start.model_dump(mode="json", exclude_none=True) == {}
    assert close.model_dump(mode="json", exclude_none=True) == {}
    assert click.model_dump(mode="json", exclude_none=True) == {"locator": {"role": "button", "name": "Sign in"}}
    assert typed.model_dump(mode="json", exclude_none=True) == {
        "locator": {"role": "textbox", "name": "Search"},
        "text": "bing.com",
        "textType": "literal",
    }
    assert wait.model_dump(mode="json", exclude_none=True) == {
        "locator": {"role": "main", "name": "Results"},
        "state": "visible",
        "timeout_ms": 5000,
    }
    with pytest.raises(ValidationError):
    ...
    with pytest.raises(ValidationError):
        WebClickOnParams.model_validate({"locator": {"role": "button", "ref": "e83"}})
    with pytest.raises(ValidationError):
        WebClickOnParams.model_validate({"target": "Sign in"})
    with pytest.raises(ValidationError):
